  ...
  try {
    const company = await prisma.company.create({
      data: {
        ...
        User: { // Link the company to a user if provided
          connect: ctx.request.body.userId ? { id: ctx.request.body.userId } : undefined,
        },
      },
    });
    ctx.body = company;
...
router.get('/:id/user', async (ctx) => {
    try {
        const id = parseInt(ctx.params.id);
        const company = await prisma.company.findUnique({where: {id}, include: {User: true}})
        if(!company){
            ctx.status = 404;
            ctx.body = {error: "Company not found"}
        } else {
            ctx.body = company.User
            ctx.status = 200;
        }
